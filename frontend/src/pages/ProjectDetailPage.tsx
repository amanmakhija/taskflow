...
import { Plus, ArrowLeft, Pencil, Check, X, SlidersHorizontal, ClipboardList } from 'lucide-react';
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8">
        <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center">
          <p className="text-destructive font-medium mb-3">Project not found or failed to load</p>
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8">
        ...
        <div className="min-w-0 flex-1">
          {editingName ? (
            <div className="flex items-center gap-2">
              ...
              />
              <button onClick={() => updateProject(newName)} className="p-1.5 rounded-md bg-primary text-primary-foreground hover:bg-primary/90">
                <Check className="h-4 w-4" />
              </button>
              <button onClick={() => setEditingName(false)} className="p-1.5 rounded-md border hover:bg-muted">
                <X className="h-4 w-4" />
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-2 group">
              <h1 className="font-display font-bold text-2xl tracking-tight truncate">{project.name}</h1>
              <button
                onClick={startEdit}
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-muted/20 py-20 text-center">
          ...
          <h3 className="font-display font-semibold text-lg mb-1">No tasks yet</h3>
          <p className="text-sm text-muted-foreground mb-6 max-w-xs">
            Add your first task to start tracking work on this project.
          </p>
          <Button onClick={() => { setEditingTask(undefined); setTaskModalOpen(true); }} className="gap-2">
            <Plus className="h-4 w-4" />
            Add first task
          </Button>
        </div>
        ...
        <div className="rounded-xl border border-border bg-muted/20 p-8 text-center">
          ...
          <button onClick={() => { setStatusFilter('all'); setPriorityFilter('all'); }} className="text-sm text-primary hover:underline mt-1">
            Clear filters
          </button>
        </div>
      ) : (
