...
export function TaskModal({ projectId, task, open, onClose }: TaskModalProps) {
  ...
  const { mutate: createTask, isPending: creating } = useMutation({
    ...
    onError: (e: any) => toast({ title: e.response?.data?.error || 'Failed to create task', variant: 'destructive' }),
  });
  ...
  const { mutate: updateTask, isPending: updating } = useMutation({
    ...
    onError: (e: any) => toast({ title: e.response?.data?.error || 'Failed to update task', variant: 'destructive' }),
  });
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-[520px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="task-desc">Description <span className="normal-case text-muted-foreground">(optional)</span></Label>
            <textarea
              id="task-desc"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="task-due">Due date <span className="normal-case text-muted-foreground">(optional)</span></Label>
            <Input
              id="task-due"
              type="date"
              value={form.due_date}
